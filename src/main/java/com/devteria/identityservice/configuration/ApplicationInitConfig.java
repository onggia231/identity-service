...
@Configuration
@RequiredArgsConstructor // Chỉ tạo constructor cho các trường final và @NonNull. Các trường không final hoặc không
// @NonNull sẽ không được bao gồm trong constructor này
@FieldDefaults(level = AccessLevel.PRIVATE, makeFinal = true)
// level = AccessLevel.PRIVATE tat ca cac truong private, makeFinal = true tat ca cac truong final
@Slf4j
public class ApplicationInitConfig {

    //    @Autowired  // su dung @RequiredArgsConstructor ko can khai @Autowired, su dung @FieldDefaults(level =
    // AccessLevel.PRIVATE ko can khai private
    ...
    // kiem tra xem (prefix + value) spring.datasource.driverClassName = (havingValue) com.mysql.cj.jdbc.Driver thi ham
    // applicationRunner duoc hoat dong
    ApplicationRunner applicationRunner(UserRepository userRepository, RoleRepository roleRepository) {
        ...
        return args -> {
            if (userRepository.findByUsername(ADMIN_USER_NAME).isEmpty()) { // kiem tra xem da co tai khoan ADMIN chua
                ...
                // tao 1 Role USER
                roleRepository.save(
                        Role.builder() // su dung builder de bao tri, mo rong, linh hoat (Luu ý: Doi tuong Role khai
                                // lombok
                                // @Builder)
