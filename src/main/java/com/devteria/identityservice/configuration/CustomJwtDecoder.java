...
public class CustomJwtDecoder implements JwtDecoder { // JwtDecoder: giải mã và xác thực JSON Web Tokens (JWT).
    ...
    // Dùng @Autowired khi bạn muốn Spring tự động tiêm phụ thuộc vào các bean của bạn (Không cần khoi tao ham
    // constructor de khai tham so authenticationService)
    // Nếu không sử dụng @Autowired thì khai báo kiểu Lombok với @RequiredArgsConstructor
    @Autowired
    private AuthenticationService authenticationService;

    private NimbusJwtDecoder nimbusJwtDecoder =
            null; // một lớp trong Spring Security OAuth 2.0 được sử dụng để giải mã và xác thực JSON Web Tokens (JWT)
